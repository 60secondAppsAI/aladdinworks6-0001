...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRooms = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRooms">
                <template slot="header">
  ...
  <base-input label="Usage" type="text" placeholder="Enter Usage" v-model="roomToAdd.usage"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="rooms" :row-key="record => record.RoomId" :pagination="pagination"
...
import RoomPictureView from './RoomPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'roomId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRoomsTable() {
      ...
      for (let i = 0; i < this.rooms.length; i++) {
        roomsTableData.push({
          ...
          year: this.rooms[i].year,
          date: this.rooms[i].date,
          competitionId: this.rooms[i].competitionId,
          roomId: this.rooms[i].roomId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-rooms',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRoomsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.roomToAdd.created = currentDate;
